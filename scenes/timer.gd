...
func start_timer():
	...
	if battle_scene:
		...
		if battle_scene and "current_bullet" in battle_scene:
			...
			if "is_player_time_frozen" in battle_scene and battle_scene.is_player_time_frozen:
				run_time = default_total_time * 0.5
				current_line_color = Color.SKY_BLUE
				battle_scene.is_player_time_frozen = false # Reset trạng thái sau khi đã phạt
